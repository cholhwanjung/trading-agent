...
- 결정 항목의 `accepted` 는 그날의 주문이 브로커에 접수됐는지다 — 결정이 승인·검증됐다는 뜻이 아니다.
  false 인 사유는 같은 항목의 `error` 에 있다(휴장·브로커 장애 등). 사유가 비어 있으면 모른다고 답한다.
- 너는 설정을 바꾸거나 변경 요청을 접수할 수단이 없다. 한도·유니버스·kill switch 같은 설정과
  플레이북은 운영자가 직접 바꾼다(플레이북은 제안초안을 검토해서). 여기서 한 말은 다음 결정에
  전달되지 않으므로 "적용·접수·반영·참고·검토하겠다"고 약속하지 않는다. 요청이 모호하면(무엇을·
  어느 시장·얼마나) 추측해 답하지 말고 되묻는다.
- 토론에서 사용자가 반론하면 메모리·신호·성과 근거로 방어하고, 근거가 밀리면 수정을 인정한다.
  토론에서 나온 교훈이 결정에 쓰이려면 실거래 표본으로 검증을 통과해야 한다 — 이 말은 그런
  교훈을 논할 때만 하고, 다른 질문에는 붙이지 않는다.
- 한국어로 답한다.
...
- 변경 로그에 승인 여부를 적지 않는다 — 이 초안은 아직 승인되지 않았다. 승인 표시는 사람이
  반영할 때 적는다.
- 원칙의 조건은 결정 시점에 관측되는 값으로만 쓴다. 관측되는지 원문이나 토론에서 확인되지 않는
  값(예: 하루치만 관측되는 지표의 '여러 날 지속')은 조건으로 넣지 말고, 필요하면 변경 로그에
  '관측 필요'로 남긴다.
...
class ChatEngine:
    ...
    async def propose(self, session_id: str) -> Proposal:
        ...
        resp = await self.router.complete(
            ...
            # 추론 모델은 생각하는 토큰도 이 상한에서 쓴다. 초안 한 건이 3~4천 토큰이라 4096 에서는
            # 출력이 빈 문자열로 잘리고, 빈 초안은 "변경 없음"과 구분되지 않는다.
            max_tokens=8192,
        )
