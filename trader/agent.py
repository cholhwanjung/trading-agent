...
def build_user_prompt(
    ...
    if budget is not None:
        payload["budget"] = {
            ...
            "max_order_weight 는 목표 수준이 아니라 **한 번에 움직이는 폭**의 상한이다 "
            "(현재 비중과 목표 비중의 차이에 걸린다. null 이면 상한 없음). 이를 넘기면 "
            "비중이 상한까지 줄어드는 것이 아니라 그 종목의 주문이 통째로 나가지 않아 "
            "**현재 비중 그대로 남는다** — 목표를 크게 옮기고 싶으면 한 번에 뛰지 말고 "
            "여러 날에 나눠 옮겨라. 현재 비중은 current_positions 의 market_value 를 "
            "equity 로 나눠 구한다.",
            **asdict(budget),
        }
    if events:
