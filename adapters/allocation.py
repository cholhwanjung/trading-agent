...
def split_account(
    total_assets: float, share: float, held: float, broker_cash: float
) -> tuple[float, float]:
    """한 계좌를 여러 시장이 공유할 때 이 시장의 (쓸 수 있는 현금, 평가액).

    시장마다 배분 벡터가 따로 있고 각각 합이 1 이라, 같은 돈을 두 시장이 각자 자기
    것으로 보면 합계 200% 를 주문하려 든다. 먼저 도는 쪽이 여력을 다 쓰고 나머지는
    통째로 스킵되므로 결과가 실행 순서에 좌우된다.

    나누는 대상은 **총자산**이다 — 현금만 나누고 보유는 각자 100% 자기 몫으로 두면
    지분이 유지되지 않는다. 100 을 사면 보유가 100 늘지만 자기 지갑에서 나가는 현금은
    지분만큼(예: 50)이고 나머지는 다른 시장 몫에서 나가기 때문이다. 그 시장은 매수할
    때마다 자기 몫이 커진다 — 지분 0.5 인 두 시장에서 한쪽이 계좌의 81.8% 로 수렴한다.

    쓸 수 있는 현금은 예산에서 자기 보유를 뺀 나머지이되, 브로커가 인정하는 여력을
    넘지 않는다 — 몫 계산이 맞아도 계좌에 그만큼의 현금이 실제로 없으면 주문은 거부된다.
    보유가 이미 예산을 넘으면 현금은 0 이고, 초과분은 다음 스텝의 배분 벡터가 매도로
    되돌린다(평가액이 곧 분모라 그 종목의 현재 비중이 1 을 넘게 계산된다).
    """
    equity = total_assets * share
    return max(0.0, min(equity - held, broker_cash)), equity
